...
/// into one of these slots; the render thread uploads the slot into the decoder's
/// input texture (portable QRhi path; works on every backend). The universal
/// fallback when no GPU-direct path is available (BlueGpuDirect is a later pass).
struct BluefishCpuCapture final : score::gfx::interop::GpuDirectCaptureStrategy
{
  ...
  const char* name() const noexcept override { return "Bluefish-CPU"; }
  ...
  void acquireForRender(QRhiResourceUpdateBatch& res) override
  {
    ...
    // Source stride = the raster's true pitch (V210 rows are 48-pixel-group
    // padded, wider than the decoder texture's texWidth*4 when w % 48 != 0,
